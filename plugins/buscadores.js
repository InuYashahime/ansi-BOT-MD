...
const {googleImage} = require('@bochilteam/scraper') 
  if (budy.toLowerCase().includes('gore') || budy.toLowerCase().includes('cp') || budy.toLowerCase().includes('porno') || budy.toLowerCase().includes('rule34') || 
      budy.toLowerCase().includes('hentai') || budy.toLowerCase().includes('xxx') || budy.toLowerCase().includes('porn') || budy.toLowerCase().includes('nsfw') ||
      budy.toLowerCase().includes('sex') || budy.toLowerCase().includes('nude') || budy.toLowerCase().includes('naked') || budy.toLowerCase().includes('erotic') ||
      budy.toLowerCase().includes('adult') || budy.toLowerCase().includes('18+') || budy.toLowerCase().includes('mature') || budy.toLowerCase().includes('explicit') ||
      budy.toLowerCase().includes('lewd') || budy.toLowerCase().includes('ecchi') || budy.toLowerCase().includes('nsfwcontent') || budy.toLowerCase().includes('nsfwart') ||
      budy.toLowerCase().includes('nsfwimage') || budy.toLowerCase().includes('nsfwvideo') || budy.toLowerCase().includes('nsfwgif') || budy.toLowerCase().includes('nsfwanime') ||
      budy.toLowerCase().includes('nsfwmeme') || budy.toLowerCase().includes('nsfwmemes') || budy.toLowerCase().includes('nsfwmanga') || budy.toLowerCase().includes('nsfwcomic') ||
      budy.toLowerCase().includes('nsfwcartoon') || budy.toLowerCase().includes('nsfwcosplay') || budy.toLowerCase().includes('nsfwcosplayer') || budy.toLowerCase().includes('nsfwcosplayers') ||
      budy.toLowerCase().includes('nsfwcosplaygirl') || budy.toLowerCase().includes('nsfwcosplayboy') || budy.toLowerCase().includes('nsfwcosplaygirls') || budy.toLowerCase().includes('nsfwcosplayboys')) {
    return m.reply('😐 NO PIDA BOLUDECES');
  }
...
if (command == 'chatgpt' || command == 'ia') {
  ...
  try {
    ...
    let sistema1 = `actuaras como un bot de WhatsApp creado por ELIASAR YT tu seras ✨ 𝐚𝐧𝐬í-𝐁𝐎𝐓 ✨ i estaras inspirado en GPT 4 i respinderas con emojis`;
    async function getOpenAIChatCompletion(texto) {
      const openaiAPIKey = global.openai_key;
      let chgptdb = global.chatgpt.data.users[m.sender];
      chgptdb.push({ role: 'user', content: texto });
      const url = "https://api.cafirexos.com/api/chatgpt?text=";
      const headers = { "Content-Type": "application/json", "Authorization": `Bearer ${openaiAPIKey}` };
      const data = { "model": "gpt-3.5-turbo", "messages": [{ "role": "system", "content": sistema1 }, ...chgptdb, ]};
      const response = await fetch(url, {method: "POST", headers: headers, body: JSON.stringify(data)});
      const result = await response.json();
      const finalResponse = result.choices[0].message.content.replace('${pushname}', m.pushname);
    ...
    try {
      ...
      try {
        const syms1 = `actuaras como un bot de WhatsApp creado por ELIASAR YT tu seras ✨ 𝐚𝐧𝐬í-𝐁𝐎𝐓 ✨ i estaras inspirado en GPT 4 i respinderas con emojis`
...
if (command == 'bard' || command == 'ia2') { 
  ...
  try {
    ...
    await m.reply(res.result.replace('${pushname}', m.pushname))
  } catch (e) {
    return m.reply(info.error)
    console.log(e)
  }
